...
int mu_coreaudio_formats_partition(int formats_n, AudioStreamRangedDescription formats[formats_n], struct Mu_AudioFormat const audioformat) {
    ...
    while (formats_good_n < formats_n) {
        ...
        if (format.mFormat.mChannelsPerFrame < audioformat.channels)
            goto known_bad;
        ...
        if (kAudioStreamAnyRate == format.mFormat.mSampleRate) {
            if (audioformat.samples_per_second < format.mSampleRateRange.mMinimum)
                goto known_bad;
            if (audioformat.samples_per_second > format.mSampleRateRange.mMaximum)
                goto known_bad;
...
MU_MACOS_INTERNAL
AudioObjectPropertyAddress const MU_MACOS_COREAUDIO_DEFAULT_OUTPUT_DEVICE_PROPERTY_ADDRESS = (AudioObjectPropertyAddress){
...
MU_MACOS_INTERNAL
OSStatus mu_coreaudio_callback(AudioDeviceID inDevice,
    ...
    if (found_outputs_n < outputs_n)
        return noErr;
    if (outputs[0].frame_n != outputs[1].frame_n)
        return noErr; // actually that's weird
    if (outputs[0].frame_n == 0)
        return noErr; // that's weird too
...
#endif
    struct Mu_AudioBuffer audiobuffer = {.samples = client_buffer, .samples_count = frame_n * audioformat.channels, .format = audioformat};
...
MU_MACOS_INTERNAL
Mu_Bool mu_audio_output_start(struct Mu *mu, struct Mu_Session *session) {
    OSStatus status = noErr;
    AudioDeviceID output_device;
    AudioObjectPropertyAddress default_output_device_property_address = (AudioObjectPropertyAddress){
            .mSelector = kAudioHardwarePropertyDefaultOutputDevice, .mScope = kAudioObjectPropertyScopeGlobal, .mElement = kAudioObjectPropertyElementMaster};
    for (UInt32 size = sizeof output_device;
         status = AudioObjectGetPropertyData(kAudioObjectSystemObject, &default_output_device_property_address, 0, NULL, &size, &output_device),
                (status != noErr || output_device == 0);) {
        mu->error = "could not obtain default audio device";
        goto error;
    }
    UInt32 streams_size;
    AudioObjectPropertyAddress streams_address = {
            .mSelector = kAudioDevicePropertyStreams, .mScope = kAudioDevicePropertyScopeOutput, .mElement = kAudioObjectPropertyElementMaster};
    ...
    for (UInt32 size = sizeof left_right_channels;
         status = AudioObjectGetPropertyData(output_device,
                                             &((AudioObjectPropertyAddress){.mSelector = kAudioDevicePropertyPreferredChannelsForStereo,
    ...
    for (int stream_i = 0; stream_i < streams_n; ++stream_i) {
        ...
        for (UInt32 size = sizeof starting_channel;
             stream_status = AudioObjectGetPropertyData(stream,
                                                        &((AudioObjectPropertyAddress){.mSelector = kAudioStreamPropertyStartingChannel,
        ...
        UInt32 formats_size;
        if (stream_status = AudioObjectGetPropertyDataSize(stream,
                                                           &((AudioObjectPropertyAddress){.mSelector = kAudioStreamPropertyAvailableVirtualFormats,
                                                                                          .mScope = kAudioObjectPropertyScopeGlobal,
                                                                                          .mElement = kAudioObjectPropertyElementMaster}),
                                                           0,
                                                           NULL,
        ...
        if (stream_status = AudioObjectGetPropertyData(stream,
                                                       &((AudioObjectPropertyAddress){.mSelector = kAudioStreamPropertyAvailableVirtualFormats,
                                                                                      .mScope = kAudioObjectPropertyScopeGlobal,
                                                                                      .mElement = kAudioObjectPropertyElementMaster}),
                                                       0,
    ...
    if (selected_stream_i == streams_n) {
        ...
        snprintf(mu->error_buffer,
                 MU_MAX_ERROR,
                 "could not obtain stream: Mu_AudioFormat{ %dhz, %d channels, %d bytes per sample }",
                 mu_audio_audioformat.samples_per_second,
                 mu_audio_audioformat.channels,
                 mu_audio_audioformat.bytes_per_sample);
        goto error_allocated_streams;
    }
    ...
    if (selected_format.mFormat.mSampleRate == kAudioStreamAnyRate) {
        ...
        for (UInt32 size = sizeof selected_format.mFormat;
             status = AudioObjectSetPropertyData(selected_stream,
                                                 &((AudioObjectPropertyAddress){.mSelector = kAudioStreamPropertyVirtualFormat,
                                                 .mScope = kAudioObjectPropertyScopeGlobal,
                                                 .mElement = kAudioObjectPropertyElementMaster}),
                                                 0,
                                                 NULL,
                                                 size,
                                                 &selected_format.mFormat),
                    status != noErr;) {
...
MU_MACOS_INTERNAL
Mu_Bool mu_audio_initialize(struct Mu *mu, struct Mu_Session *session) {
    OSStatus status = noErr;
    if (!mu->audio.callback)
        mu->audio.callback = mu_audio_emit_silence;

    if (AudioObjectAddPropertyListener(
                kAudioObjectSystemObject, &MU_MACOS_COREAUDIO_DEFAULT_OUTPUT_DEVICE_PROPERTY_ADDRESS, mu_coreaudio_property_listener, session),
        status != noErr) {
...
MU_MACOS_INTERNAL
void mu_audio_output_close(struct Mu *mu, struct Mu_Session *session) {
    if (session->DeviceID && session->IOProcID) {
        if (AudioDeviceStop(session->DeviceID, mu_coreaudio_callback) != noErr || AudioDeviceDestroyIOProcID(session->DeviceID, session->IOProcID) != noErr) {
...
Mu_Bool mu_time_initialize(struct Mu *mu, struct Mu_Session *session) {
    if (mach_timebase_info(&session->timebase))
        return MU_FALSE;
...
#undef get_opt
    ...
    /* setup window */ {
        Mu_NSWindowStyleMask const window_style_mask = Mu_NSWindowStyleMaskTitled | Mu_NSWindowStyleMaskClosable | Mu_NSWindowStyleMaskMiniaturizable |
                                                       Mu_NSWindowStyleMaskResizable | Mu_NSWindowStyleMaskTexturedBackground;
        ...
        if (window_x == 0 || window_y == 0) {
            ...
            if (window_x != 0)
                content_rect.origin.x = window_x;
            if (window_y != 0)
                content_rect.origin.y = window_y;
    ...
    /* setup content view */ {
        Mu_OpenGLView *view = [[[Mu_OpenGLView alloc] initWithFrame:NSMakeRect(window_x, window_y, window_width, window_height)
                                                        pixelFormat:[Mu_OpenGLView defaultPixelFormat]] autorelease];
...
void mu_update_stick(struct Mu_Stick *stick, float const x, float const y) {
    stick->x = x;
    stick->y = y;
    if (fabs(stick->x) <= stick->threshold)
        stick->x = 0.0f;
    if (fabs(stick->y) <= stick->threshold)
        stick->y = 0.0f;
}
...
MU_MACOS_INTERNAL
void mu_gamepad_hid_update_stick(struct Mu_Stick *stick,
                                 struct Mu_Gamepad_HID_Mapping_Stick const *mapping,
                                 struct Mu_HIDAddress address,
...
MU_MACOS_INTERNAL
void mu_gamepad_hid_update_analog_button(struct Mu_AnalogButton *analog_button,
                                         struct Mu_Gamepad_HID_Mapping_AnalogButton const *mapping,
                                         float const scaled_value) {
...
void mu_gamepad_hid_input_value_callback(void *context, IOReturn result, void *sender, IOHIDValueRef value) {
    ...
    struct Mu_HIDAddress address = {.usage_page = IOHIDElementGetUsagePage(element), .usage = IOHIDElementGetUsage(element)};

#define X(button_name)                                                                                                                                         \
    if (mu_gamepad_hidaddress_equals(mapping->button_name.address, address))                                                                                   \
        mu_gamepad_hid_update_digital_button(&gamepad->button_name, &mapping->button_name, state);
    MU_GAMEPAD_DIGITAL_BUTTONS_XENUM;
#undef X

#define X(analog_button_name)                                                                                                                                  \
    if (mu_gamepad_hidaddress_equals(mapping->analog_button_name.address, address))                                                                            \
        mu_gamepad_hid_update_analog_button(&gamepad->analog_button_name, &mapping->analog_button_name, scaled_value);
    MU_GAMEPAD_ANALOG_BUTTONS_XENUM;
#undef X
...
struct Mu_Gamepad_HID_Mapping const mu_ds4_mapping = {
        ...
        .b_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x03},
                },
        .x_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x01},
                },
        .y_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x04},
                },
        ...
        .left_thumb_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0xB},
                },
        .right_thumb_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0xC},
                },
        .back_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x9},
                }, // share button
        .start_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0xA},
                }, // option button
        ...
        .right_button = {.address = {.usage_page = kHIDPage_GenericDesktop, .usage = 0x39},
                         .states_n = 3,
                         .states =
                                 {
                                         3,
        ...
        .left_trigger = {.address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_Rx}, .xmin = 0.0f, .xmax = 255.0f},
        .right_trigger = {.address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_Ry}, .xmin = 0.0f, .xmax = 255.0f},
        .left_thumb_stick =
                {
                        .x_address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_X},
                        .y_address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_Y},
                        .xmin = 0.0f,
                        .xmax = 255.0f,
                        .ymin = 255.0f,
                        .ymax = 0.0f,
        ...
        .right_thumb_stick =
                {
                        .x_address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_Z},
                        .y_address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_Rz},
                        .xmin = 0.0f,
                        .xmax = 255.0f,
                        .ymin = 255.0f,
                        .ymax = 0.0f,
...
struct Mu_Gamepad_HID_Mapping const mu_huijia_3_0xe8f_0x3013_mapping = {
        ...
        .b_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x02},
                },
        .x_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x04},
        ...
        .left_shoulder_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x07},
                },
        .right_shoulder_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0x08},
                },
        .left_thumb_button =
                {
                        .address = {.usage_page = kHIDPage_Button, .usage = 0xB},
        ...
        .left_trigger = {.address = {.usage_page = kHIDPage_Button, .usage = 0x05}, .xmin = 0.0f, .xmax = 255.0f},
        .right_trigger = {.address = {.usage_page = kHIDPage_Button, .usage = 0x06}, .xmin = 0.0f, .xmax = 255.0f},
        .left_thumb_stick =
                {
                        .x_address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_X},
                        .y_address = {.usage_page = kHIDPage_GenericDesktop, .usage = kHIDUsage_GD_Y},
...
Mu_Bool mu_gamepad_initialize(struct Mu *mu, struct Mu_Session *session) {
    ...
    for (NSEnumerator *device_enumerator = [devices objectEnumerator]; !mu->gamepad.connected && (device = (IOHIDDeviceRef)[device_enumerator nextObject]);) {
...
#define X(button_name)                                                                                                                                         \
    if (mu->gamepad.button_name.pressed)                                                                                                                       \
        MU_MACOS_TRACEF("pressed: " #button_name "\n");
     MU_GAMEPAD_DIGITAL_BUTTONS_XENUM;
     MU_GAMEPAD_ANALOG_BUTTONS_XENUM;
#undef X
#endif
...
Mu_Bool Mu_Initialize(struct Mu *mu) {
    ...
    @autoreleasepool {
        if (!mu_time_initialize(mu, session))
            return MU_FALSE;
        if (!mu_application_initialize(mu, session))
            return MU_FALSE;
        if (!mu_window_initialize(mu, session))
            return MU_FALSE;
        if (!mu_audio_initialize(mu, session))
            return MU_FALSE;
        if (!mu_gamepad_initialize(mu, session))
            return MU_FALSE;
...
#if MU_MACOS_RUN_MODE == MU_MACOS_RUN_MODE_COROUTINE
    if (!mu_fibers_initialize(mu, session))
        return MU_FALSE;
#endif
    return MU_TRUE;
}
...
Mu_Bool mu_nsevent_process(struct Mu *const mu, struct Mu_Session const *const session, NSEvent const *const event) {
    ...
    case Mu_NSEventTypeFlagsChanged: {
        ...
        switch (keyCode) {
        case MU_CTRL:
            is_down = (flags & Mu_NSEventModifierFlagControl) ? MU_TRUE : MU_FALSE;
            e_p = 1;
            break;
        case MU_CMD:
            is_down = (flags & Mu_NSEventModifierFlagCommand) ? MU_TRUE : MU_FALSE;
            e_p = 1;
            break;
        case MU_OPTION:
            is_down = (flags & Mu_NSEventModifierFlagOption) ? MU_TRUE : MU_FALSE;
            e_p = 1;
            break;
        case MU_SHIFT:
            is_down = (flags & Mu_NSEventModifierFlagShift) ? MU_TRUE : MU_FALSE;
            e_p = 1;
            break;
        }
    ...
    default: return MU_FALSE; // events we don't know about
    }
    return MU_TRUE;
}
...
- (void)viewWillStartLiveResize {
    assert(!nstimer);
    nstimer = [NSTimer timerWithTimeInterval:0.001 target:self selector:@selector(interruptMainLoop) userInfo:nil repeats:YES];
    ...
    @autoreleasepool {
        ...
        for (NSEvent *event; (event = [NSApp nextEventMatchingMask:Mu_NSEventMaskAny untilDate:deadline inMode:NSDefaultRunLoopMode dequeue:YES]);) {
            if (events_n == events_capacity) {
                ...
                if (was_on_stack)
                    memcpy(events, events_buffer, sizeof events_buffer);
            }
...
#undef X
    ...
    NSRect contentRect = [session->cocoa_resources.window convertRectToScreen:[session->opengl_view frame]];
    ...
    mu->window.resized = old_window.size.x != mu->window.size.x || old_window.size.y != mu->window.size.y;
...
Mu_Bool Mu_LoadAudio(const char *filename, struct Mu_AudioBuffer *audio) {
    ...
    /* read all and convert */ {
        ...
        if (st != noErr && st != kAudioFileEndOfFileError)
            goto error;
    }
