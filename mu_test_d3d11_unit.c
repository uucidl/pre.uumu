...
int main(int argc, char **argv)
{
     ...
     struct ID3D11RasterizerState *rasterizer_state = NULL;
     {
         ...
         D3D11_RASTERIZER_DESC desc = {
             ...
             .CullMode = D3D11_CULL_NONE, /* move to front later */
         };
     ...
     struct ID3D11InputLayout *imm_layout;
     {
          D3D11_INPUT_ELEMENT_DESC descs[] = {
               { .SemanticName = "POSITION", .Format = DXGI_FORMAT_R32G32B32_FLOAT, .AlignedByteOffset = offsetof(struct FlatVertex2d, posx), .InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA },
               { .SemanticName = "COLOR", .Format = DXGI_FORMAT_R32G32B32_FLOAT, .AlignedByteOffset = offsetof(struct FlatVertex2d, colr), .InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA },
          };
     ...
     while (Mu_Pull(&mu)) {
         if (cb) buf__hdr(cb)->len = 0;
         if (vb) buf__hdr(vb)->len = 0;
         if (ib) buf__hdr(ib)->len = 0;
         ...
         struct ID3D11RenderTargetView *output_view = NULL;
         {
          ...
          /* dropped frame indicator */ {
               ...
               buf_push(cb, ((struct GfxQuadCommand){
                            .x0 = 10, .x1 = 10+100, .y0=cy, .y1=cy+100,
                            .r=colr, .g=colg, .b=colb, .a=1.0f,
                             }));
          }
          cy += 100;
          ...
          COM_CALL(mu.win32->d3d11_device_context, RSSetState, rasterizer_state);
          {
               D3D11_VIEWPORT viewport = {
                    .TopLeftX = 0, .TopLeftY = 0, .Width = mu.window.size.x, .Height = mu.window.size.y, .MinDepth = 0.0, .MaxDepth = 1.0,
               };
